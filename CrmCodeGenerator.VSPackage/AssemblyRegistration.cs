...
namespace CrmPluginRegExt.VSPackage
{
	...
	internal class AssemblyRegistration : INotifyPropertyChanged
	{
		...
		public AssemblyRegistration(XrmServiceContext context, IOrganizationService service)
		{
			Context = context;
			Service = service;
		}
		...
		/// </summary>
		internal void UpdateAssembly(Guid assemblyId, bool? sandbox)
		{
			lock (ActionLock)
			{
				Id = assemblyId;
		...
		private void DeleteTree(Guid objectId, Dependency.Enums.RequiredComponentType type)
		{
			var dependencies = ((RetrieveDependenciesForDeleteResponse)
			                    Context.Execute(new RetrieveDependenciesForDeleteRequest
			                                    {
			...
			dependencies.ForEach(entity =>
			                     {
				                     ...
				                     {
					                     UpdateStatus(
						                     $"Deleting dependency '{entityQ.DependentComponentObjectId.Value}' of type '{(Dependency.Enums.RequiredComponentType) entityQ.DependentComponentType.Value}' ... ", 1);
			...
			Context.Execute(new DeleteRequest
			                {
